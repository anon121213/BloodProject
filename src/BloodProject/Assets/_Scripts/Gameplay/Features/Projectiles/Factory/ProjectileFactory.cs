...
namespace _Scripts.Gameplay.Features.Projectiles.Factory
{
  ...
  {
    public GameEntity CreateSimpleBulletProjectile(ProjectileConfig projectileConfig,
    ...
    public GameEntity CreateTeleportProjectile(ProjectileConfig projectileConfig,
      int producerID, Vector3 startPoint, Vector3 endPoint)
    {
      return CreateEntity.Empty()
        .AddId(IdentifierService.Next())
        .AddProducerId(producerID)
        .AddWorldPosition(startPoint)
        .AddTeleportProjectileEndPoint(endPoint)
        .AddViewReference(projectileConfig.Prefab)
        .AddSelfDestructTimer(projectileConfig.LifeTime)
        .With(x => x.isTeleportProjectile = true)
        .With(x => x.isProjectile = true)
        .With(x => x.isTeleport = true)
        .With(x => x.isRotate = true)
        .With(x => x.isMovementAvailable = true);
